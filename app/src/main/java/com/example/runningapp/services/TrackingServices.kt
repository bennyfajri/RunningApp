...
class TrackingServices : LifecycleService() {
    ...
    @Inject
    lateinit var fusedLocationPriorityClient : FusedLocationProviderClient
    ...
    override fun onCreate() {
        ...
        postInitialValues()
        fusedLocationPriorityClient = FusedLocationProviderClient(this)
    ...
    private fun startTimer() {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            while(isTracking.value!!) {
                // time difference between noew and timesStarted
    ...
    private fun updateLocationTracking(isTracking: Boolean){
        if(isTracking) {
            if(TrackingUtility.hasLocationPermissions(this)){
                ...
                fusedLocationPriorityClient.requestLocationUpdates(
                    request,
                    locationCallback,
                    Looper.getMainLooper()
                )
            }else{
                fusedLocationPriorityClient.removeLocationUpdates(locationCallback)
            }
        }
    }
